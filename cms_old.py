...
def close_vlc():
    ...
    subprocess.run(
        ['taskkill', '/IM', 'vlc.exe', '/F'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
    )
...
def main():
    ...
    while True:
        ...
        user_input = (
            input(
                "Enter channels (space-separated) to change, 'r' to refresh VLC windows, or [ENTER] to quit:/n "
            )
            .strip()
            .lower()
        )
